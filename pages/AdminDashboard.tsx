...
import { Plus, Edit2, Trash2, CheckCircle, XCircle, Loader2, Calendar, Banknote, Image as ImageIcon, FileText, Users, Star } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'trips' | 'partners' | 'testimonials'>('trips');
  const [trips, setTrips] = useState<Trip[]>([]);
  const [loading, setLoading] = useState(true);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingTrip, setEditingTrip] = useState<Trip | null>(null);
  const [managingDeparturesTrip, setManagingDeparturesTrip] = useState<Trip | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsFormOpen(false);
      setEditingTrip(null);
      setFormData({ title: '', price: 0, date: '', description: '', image_url: '', program: '', is_completed: false });
      fetchTrips();
    } catch (err: any) {
  ...
  const toggleStatus = async (trip: Trip) => {
    try {
      const { error } = await supabase
        ...
        .eq('id', trip.id);
      if (error) throw error;
      toast.success(`Trip marked as ${!trip.is_completed ? 'Completed' : 'Upcoming'}`);
      fetchTrips();
    } catch (err: any) {
      toast.error(err.message);
    }
          ...
          <>
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
              <div>
                ...
                <p className="text-gray-500">Manage your travel catalog, prices, and status.</p>
              </div>
              <button 
                onClick={() => {
                  setEditingTrip(null);
            ...
            <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
              <div className="p-8">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-bold text-gray-900">Manage Departure Dates</h2>
                  <button onClick={() => setManagingDeparturesTrip(null)} className="text-gray-400 hover:text-gray-600">
                    <XCircle size={24} />
                  </button>
                </div>
        ...
        {isFormOpen && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
              <div className="p-8">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-bold text-gray-900">{editingTrip ? 'Edit Trip' : 'Create New Trip'}</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="col-span-full">
                      <label className="block text-sm font-semibold text-gray-700 mb-1 flex items-center gap-1">
                        <FileText size={14} /> Travel Program (Day-by-day itinerary)
                      </label>
                      <textarea
                        rows={12}
                        ...
                        dir="rtl"
                      ></textarea>
                      <p className="text-xs text-gray-500 mt-1">يمكنك استخدام الرموز التعبيرية والتنسيق لجعل البرنامج أكثر جاذبية</p>
                    </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
          {loading ? (
                    ...
                    <tr key={trip.id} className="hover:bg-gray-50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <img 
                            src={trip.image_url || 'https://picsum.photos/seed/travel/100/100'} 
                            alt={trip.title}
                            className="w-12 h-12 rounded-lg object-cover"
                          />
                          <div>
                            <div className="font-bold text-gray-900">{trip.title}</div>
                            <div className="text-xs text-gray-400 max-w-[200px] truncate">{trip.description}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="font-semibold text-orange-600">{trip.price} MAD</span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        ...
                        >
                          {trip.is_completed ? <CheckCircle size={14} /> : <Calendar size={14} />}
            ...
            <div className="p-20 text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-1">No trips yet</h3>
              <p className="text-gray-500">Create your first trip to see it listed here.</p>
            </div>
          )}
        </div>
          </>
